...
There are some useful `control` arguments in the `joint` function:
...
* `dic` returns the Deviance Information Criterion if set to TRUE.

```{r M1_2, cache=TRUE}
M1 <- joint(formLong = serBilir ~ year + drug  +  (1|id),
            dataLong = Longi, id = "id", timeVar = "year", 
            family = "gaussian", control=list(dic=T, waic=T, cpo=T))
sumstats <- summary(M1)
sumstats$dic
```

* `waic` returns the Widely Applicable Bayesian Information Criterion if set to TRUE.

```{r waic}
sumstats$waic
```

* `cpo` returns the Conditional Predictive Ordinate and the Predictive Integral Transform (PIT) if set to TRUE.

```{r Fig1, echo=TRUE, fig.height=3, fig.width=7}
par(mfrow=c(1,2))
hist(sumstats$cpo, main="Conditional Predictive Ordinate")
hist(sumstats$pit, main="Predictive Integral Transform")
```
For models with multiple outcomes, the cpo and pit are given as a vector with the values for each marker, therefore to have the values for the first marker, one must extract the first N values, for the second marker the N+1:2N values, ...
...
M4 <- joint(formSurv = list(DTH ~ sex + drug,
            ...
            dataLong = Longi, id = "id", timeVar = "year", family = "gaussian", 
            basRisk = c("rw1", "rw1"), assoc = c("CV", "CV"), int.strategy="eb")
summary(M4)
```
