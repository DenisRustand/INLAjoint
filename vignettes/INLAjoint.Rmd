...
&= \beta_0 + (\beta_1 + b_{i1})year_{ij} + (\beta_2 + b_{i2})year_{ij}^2 \\
...
M3 <- joint(formSurv = DTH ~ drug,
            formLong = serBilir ~ (1 + year + f1(year) + f2(year))*drug +
                       (year + f1(year) + f2(year) |id), family = "lognormal",
...
plotM3$Random + geom_abline(slope=0, intercept=0, linetype='dashed')
```
